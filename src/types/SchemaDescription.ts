...
export default class SchemaDescription {
  ...
  addChildItem({ keys }: { keys: string[] }):void {
    let clonedSchema = _.clone(this.schema);
    const currentField = _.get(clonedSchema, keys);
    if (currentField !== undefined) {
      clonedSchema = _.update(clonedSchema, keys, (n) => 
        _.concat(n, getDefaultSchema('string')),
      );
    }
  ...
  deleteItem({ keys }: { keys: string[] }): void {
    const clonedSchema = _.clone(this.schema);
    const parentKeys: string[] = getParentKey(keys);
    const parentData = parentKeys.length ? _.get(clonedSchema, parentKeys) : this.schema;
    const removeIdx = keys.pop();
    parentData.splice(removeIdx, 1);
    this.schema = clonedSchema;
  }
  ...
  addItem({ keys }: { keys: string[] }): void {
    const clonedSchema = _.clone(this.schema);
    const newSchema = _.update(clonedSchema, keys, (n) => 
      _.concat(n, getDefaultSchema('string')),
    );
  ...
  changeType({ keys, value }: { keys: string[]; value: string }): void {
    const parentKeys: string[] = getParentKey(keys); // 获取当前 key 的所有父 key
    const parentData = parentKeys.length ? _.get(this.schema, parentKeys) : this.schema; // 获取父 key 数据
    if (parentData.type === value) { // 点击自身返回
      return;
    }
    const clonedSchema = _.clone(this.schema); // 克隆数据
    ...
    if (parentKeys.length === 0) {
      ...
      this.schema = _.set(clonedSchema, parentKeys, newParentDataItem); // 插入新数据
    }
  }
